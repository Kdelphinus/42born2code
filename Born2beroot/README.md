...
2. 최초 접속일 경우, 서버로부터 전달받은 공개키를 로컬에 저장할 지 물어보고 동의 시, 저장한다.
3. 클라이언트는 해당 서버에 접속하기 위해 사용할 key pair(비대칭키)의 id를 전송한다.
4. 서버는 해당 id에 매칭되는 공개키를 발급했는지 확인한다.
5. 확인되었다면 난수 값을 생성하고 이를 해당 id의 공개키로 암호화하여 클라이언트에게 전송한다.
6. 클라이언트는 전달받은 암호화된 난수 값을 해당 개인키로 복호화한다.
7. 복호화한 난수를 이용하여 MD5 해시 값을 계산하여 서버로 전송한다.
8. 서버도 생성한 난수 값을 이용하여 MD5 해시 값을 계산하고 받은 값과 비교한다.
9. 같다면 접속을 허용한다.
