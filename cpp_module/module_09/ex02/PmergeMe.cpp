...
size_t PmergeMe::getInsertPos(size_t insertPos) {
    if (insertPos < 2)
        return _jacobsthal[insertPos];
    if (insertPos - 1 == _jacobsthal[_jacobsthalIndex]) {
        _jacobsthalIndex++;
...
void PmergeMe::vectorInsertion(size_t pairCount,
    ...
    for (size_t i = 0; i < subChainSize; i++) {
        insertPos = std::min(getInsertPos(insertPos), subChainSize);
        vectorBinaryInsertion(insertPos - 1, pairSize, mainChain, subChain);
    }
...
void PmergeMe::dequeInsertion(size_t pairCount,
    ...
    for (size_t i = 0; i < subChainSize; i++) {
        insertPos = std::min(getInsertPos(insertPos), subChainSize);
        dequeBinaryInsertion(insertPos - 1, pairSize, mainChain, subChain);
    }
    for (size_t i = 0; i < mainChain.size(); i++)
