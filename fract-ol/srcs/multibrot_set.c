...
/*   Updated: 2022/10/06 17:36:32 by myko             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	multibrot_calculation(t_comp *comp, double d, int max_r)
{
	double	x;
	double	y;
	int		n;
...
void	multibrot_draw(t_frac *frac)
{
	...
	while (frac->comp->c_i <= frac->c_bd + frac->move_ud)
	{
		while (frac->comp->c_r <= frac->c_bd + frac->move_rl)
		{
			...
			value = multibrot_calculation(frac->comp, frac->d, frac->max_r);
